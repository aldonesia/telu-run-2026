...
export default function Rules() {
  ...
  return (
    <section id="rules" className="py-16 lg:py-20 bg-gradient-to-b from-white to-gray-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 lg:mb-16 animate-fade-in-up">
          <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-[#450099]/10 to-[#FF0020]/10 rounded-full mb-4 mx-auto">
            ...
            <span className="text-sm font-semibold text-[#450099]">Wajib Dibaca</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
          {rulesData.map((section, index) => (
            <div
              key={index}
              className="group relative bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer"
            ...
            >
              ...
              <div className="w-full p-5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 ${section.iconBg} backdrop-blur-sm rounded-xl flex items-center justify-center flex-shrink-0 shadow-md group-hover:scale-110 transition-transform duration-300`}>
                    <section.icon className={section.iconColor} size={24} />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 group-hover:text-[#450099] transition-colors duration-300">{section.title}</h3>
                  </div>
                </div>
                {expandedStates[index] ? (
                  <ChevronUp className="text-gray-400 flex-shrink-0 group-hover:text-[#450099] transition-colors duration-300" size={20} />
                ) : (
                  <ChevronDown className="text-gray-400 flex-shrink-0 group-hover:text-[#450099] transition-colors duration-300" size={20} />
                )}
              </div>
                    ...
                    {section.isSchedule && section.schedule ? (
                      <div className="space-y-3">
                        {section.schedule.map((item, idx) => (
                          <div
                            key={idx}
                            className="relative bg-gradient-to-r from-gray-50 to-white p-4 rounded-lg border border-gray-200 hover:border-[#9C2163]/30 transition-all duration-300"
                          >
                      ...
                      <div className="space-y-2">
                        {section.rules?.map((rule, idx) => (
                          <div
                            key={idx}
                            className="relative flex items-start gap-3 p-2.5 rounded-lg hover:bg-gradient-to-r hover:from-[#450099]/5 hover:via-[#9C2163]/5 hover:to-[#FF0020]/5 transition-all duration-300"
                          >
                            <div className="flex-shrink-0 mt-0.5">
                              <div className={`w-6 h-6 bg-gradient-to-br ${section.color} rounded-full flex items-center justify-center shadow-sm`}>
        ...
        <div className="mt-12 lg:mt-16">
          <div className="max-w-4xl mx-auto">
            <div className="relative bg-gradient-to-br from-[#450099] via-[#9C2163] to-[#FF0020] text-white rounded-2xl overflow-hidden shadow-xl border border-white/10">
              ...
              <div className="relative z-10 p-6 lg:p-8">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h4 className="font-bold text-xl mb-3">Perhatian Penting!</h4>
